 ...
 */
public class WPPost: Decodable, Hashable, Identifiable {
    ...
    public struct BetterFeaturedImage : Codable {
        let source : String?
    ...
    public let tags: [Int]
    public let better_featured_image : BetterFeaturedImage?
    ...
    private class func getPosts(web: WPWeb, page: Int, postPerPage: Int, author: Int = 0, after date: String? = nil, categories: [Int] = [], completionHandler: @escaping ([WPPost], HTTPURLResponse)->()) {
        ...
        let networkManager = NetworkResponse<[WPPost]>(request: request)
    ...
    public class func search(web: WPWeb, query: String, completionHandler: @escaping ([WPPost])->()) {
        let urlStr = web.url + "/wp-json/wp/v2/posts?search=\(query)"
    ...
    public class func getPostsSearch(_ allPosts: [WPPost], query: String) -> [WPPost] {
        return allPosts.filter {$0.content.rendered.clean().contains(query)}
    }
